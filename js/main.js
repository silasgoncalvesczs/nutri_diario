...
window.switchSettingsTab = (tab) => {
    ...
    const btnGoals = document.getElementById('btn-tab-goals');

    if (!tabProfile || !tabGoals) return;

    // As classes que definem o botão selecionado e o botão apagado
    const activeClasses = "flex-1 py-2 text-sm font-bold rounded-lg bg-white text-primary-600 shadow-sm dark:bg-dark-surface dark:text-primary-400 transition-all";
    const inactiveClasses = "flex-1 py-2 text-sm font-bold rounded-lg text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-all";

    if (tab === 'profile') {
        tabProfile.classList.remove('hidden');
        tabProfile.classList.add('block');
        tabGoals.classList.remove('block');
        tabGoals.classList.add('hidden');

        btnProfile.className = activeClasses;
        btnGoals.className = inactiveClasses;
    } else {
        tabGoals.classList.remove('hidden');
        tabGoals.classList.add('block');
        tabProfile.classList.remove('block');
        tabProfile.classList.add('hidden');

        btnGoals.className = activeClasses;
        btnProfile.className = inactiveClasses;
    }
};
